...
echo "=== POSTDEPLOY: Collecting static files at $(date '+%c %Z') ===" | sudo tee -a /var/log/collectstatic.log
...
if [ -f "/var/app/current/staticfiles/admin/js/vendor/select2/select2.full.js" ]; then
    ...
    ls -la /var/app/current/staticfiles/admin/js/vendor/select2/select2.full.js | sudo tee -a /var/log/collectstatic.log
else
    echo "❌ ERROR: select2.full.js NOT found!" | sudo tee -a /var/log/collectstatic.log
fi

if [ -f "/var/app/current/staticfiles/admin/css/vendor/select2/select2.min.css" ]; then
    echo "✅ SUCCESS: admin select2.min.css found" | sudo tee -a /var/log/collectstatic.log
else
    echo "❌ ERROR: admin select2.min.css NOT found!" | sudo tee -a /var/log/collectstatic.log
fi
...
if [ -f "/var/app/current/staticfiles/autocomplete_light/autocomplete_light.min.js" ]; then
    ...
    ls -la /var/app/current/staticfiles/autocomplete_light/autocomplete_light.min.js | sudo tee -a /var/log/collectstatic.log
else
...
# List all autocomplete_light files for debugging
echo "All autocomplete_light files:" | sudo tee -a /var/log/collectstatic.log
find /var/app/current/staticfiles/autocomplete_light/ -type f 2>/dev/null | sudo tee -a /var/log/collectstatic.log
...
echo "=== POSTDEPLOY: Collectstatic completed at $(date '+%c %Z') ===" | sudo tee -a /var/log/collectstatic.log